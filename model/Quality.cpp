...
bool Quality::isValueValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Quality::setValue(QString input) {
...
void Quality::initValue() {
  valueValue = QString();
}
Quality * Quality::fromElement(QDomElement &element) {
  Quality *quality_ = new Quality();
  QDomNodeList list_ = element.childNodes();
  for (int i = 0; i < list_.size(); i++) {
    if (list_.at(i).isElement()) {
      QDomElement e = list_.at(i).toElement();
      quality_->addSubElement(e);
    }
  }
  QDomNamedNodeMap attrs_ = element.attributes();
  for(int i = 0; i<attrs_.size(); i++) {
    QDomAttr a = attrs_.item(i).toAttr();
    quality_->setAttribute(a);
  }
  quality_->setValue(element.text());
  return quality_;
}
void Quality::addSubElement( QDomElement &element) {
}
void Quality::setAttribute( QDomAttr &attr) {
  if(attr.localName().compare("Value", Qt::CaseInsensitive)==0) {
    setValue(attr.value());
    return;
  }
}
QDomDocument * Quality::loadXmlDocument(QFile & file) {
  if (!file.exists()) {
...
Quality  * Quality::fromFile(QFile & file) {
  QDomDocument *doc = Quality::loadXmlDocument(file);
  if(doc==NULL) {
    return NULL;
  }
  QDomElement root = doc->documentElement();
  return Quality::fromElement(root);
}
