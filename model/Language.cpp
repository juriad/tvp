...
bool Language::isValueValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Language::setValue(QString input) {
...
void Language::initValue() {
  valueValue = QString();
}
bool Language::isLangValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Language::setLang(QString input) {
    if(input.trimmed().isEmpty()) {
    return;
  }
  QString normalized = input.replace(QChar(9), QChar(32)).replace(QChar(10), QChar(32)).replace(QChar(13), QChar(32));
...
void Language::initLang() {
  langValue = QString();
}
Language * Language::fromElement(QDomElement &element) {
  Language *language_ = new Language();
  QDomNodeList list_ = element.childNodes();
  for (int i = 0; i < list_.size(); i++) {
    if (list_.at(i).isElement()) {
      QDomElement e = list_.at(i).toElement();
      language_->addSubElement(e);
    }
  }
  QDomNamedNodeMap attrs_ = element.attributes();
  for(int i = 0; i<attrs_.size(); i++) {
    QDomAttr a = attrs_.item(i).toAttr();
    language_->setAttribute(a);
  }
  language_->setValue(element.text());
  return language_;
}
void Language::addSubElement( QDomElement &element) {
}
...
QDomDocument * Language::loadXmlDocument(QFile & file) {
  if (!file.exists()) {
...
Language  * Language::fromFile(QFile & file) {
  QDomDocument *doc = Language::loadXmlDocument(file);
  if(doc==NULL) {
    return NULL;
  }
  QDomElement root = doc->documentElement();
  return Language::fromElement(root);
}
