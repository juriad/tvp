...
bool Guest::isValueValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Guest::setValue(QString input) {
...
void Guest::initValue() {
  valueValue = QString();
}
Guest * Guest::fromElement(QDomElement &element) {
  ...
  for (int i = 0; i < list_.size(); i++) {
    if (list_.at(i).isElement()) {
      ...
      guest_->addSubElement(e);
    }
  }
  QDomNamedNodeMap attrs_ = element.attributes();
  for(int i = 0; i<attrs_.size(); i++) {
    QDomAttr a = attrs_.item(i).toAttr();
    guest_->setAttribute(a);
  }
  guest_->setValue(element.text());
  return guest_;
}
void Guest::addSubElement( QDomElement &element) {
}
void Guest::setAttribute( QDomAttr &attr) {
  if(attr.localName().compare("Value", Qt::CaseInsensitive)==0) {
    setValue(attr.value());
    return;
  }
}
QDomDocument * Guest::loadXmlDocument(QFile & file) {
  if (!file.exists()) {
...
Guest  * Guest::fromFile(QFile & file) {
  QDomDocument *doc = Guest::loadXmlDocument(file);
  if(doc==NULL) {
    return NULL;
  }
  QDomElement root = doc->documentElement();
  return Guest::fromElement(root);
}
