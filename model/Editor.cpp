...
bool Editor::isValueValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Editor::setValue(QString input) {
...
void Editor::initValue() {
  valueValue = QString();
}
Editor * Editor::fromElement(QDomElement &element) {
  Editor *editor_ = new Editor();
  QDomNodeList list_ = element.childNodes();
  for (int i = 0; i < list_.size(); i++) {
    if (list_.at(i).isElement()) {
      QDomElement e = list_.at(i).toElement();
      editor_->addSubElement(e);
    }
  }
  QDomNamedNodeMap attrs_ = element.attributes();
  for(int i = 0; i<attrs_.size(); i++) {
    QDomAttr a = attrs_.item(i).toAttr();
    editor_->setAttribute(a);
  }
  editor_->setValue(element.text());
  return editor_;
}
void Editor::addSubElement( QDomElement &element) {
}
void Editor::setAttribute( QDomAttr &attr) {
  if(attr.localName().compare("Value", Qt::CaseInsensitive)==0) {
    setValue(attr.value());
    return;
  }
}
QDomDocument * Editor::loadXmlDocument(QFile & file) {
  if (!file.exists()) {
...
Editor  * Editor::fromFile(QFile & file) {
  QDomDocument *doc = Editor::loadXmlDocument(file);
  if(doc==NULL) {
    return NULL;
  }
  QDomElement root = doc->documentElement();
  return Editor::fromElement(root);
}
