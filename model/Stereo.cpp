...
bool Stereo::isValueValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Stereo::setValue(QString input) {
...
void Stereo::initValue() {
  valueValue = QString();
}
Stereo * Stereo::fromElement(QDomElement &element) {
  Stereo *stereo_ = new Stereo();
  QDomNodeList list_ = element.childNodes();
  for (int i = 0; i < list_.size(); i++) {
    if (list_.at(i).isElement()) {
      QDomElement e = list_.at(i).toElement();
      stereo_->addSubElement(e);
    }
  }
  QDomNamedNodeMap attrs_ = element.attributes();
  for(int i = 0; i<attrs_.size(); i++) {
    QDomAttr a = attrs_.item(i).toAttr();
    stereo_->setAttribute(a);
  }
  stereo_->setValue(element.text());
  return stereo_;
}
void Stereo::addSubElement( QDomElement &element) {
}
void Stereo::setAttribute( QDomAttr &attr) {
  if(attr.localName().compare("Value", Qt::CaseInsensitive)==0) {
    setValue(attr.value());
    return;
  }
}
QDomDocument * Stereo::loadXmlDocument(QFile & file) {
  if (!file.exists()) {
...
Stereo  * Stereo::fromFile(QFile & file) {
  QDomDocument *doc = Stereo::loadXmlDocument(file);
  if(doc==NULL) {
    return NULL;
  }
  QDomElement root = doc->documentElement();
  return Stereo::fromElement(root);
}
