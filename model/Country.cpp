...
bool Country::isValueValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Country::setValue(QString input) {
...
void Country::initValue() {
  valueValue = QString();
}
bool Country::isLangValid(QString input) {
  bool ok;
  ok = !input.trimmed().isEmpty();
  return ok;
}
void Country::setLang(QString input) {
    if(input.trimmed().isEmpty()) {
    return;
  }
  QString normalized = input.replace(QChar(9), QChar(32)).replace(QChar(10), QChar(32)).replace(QChar(13), QChar(32));
...
void Country::initLang() {
  langValue = QString();
}
Country * Country::fromElement(QDomElement &element) {
  ...
  for (int i = 0; i < list_.size(); i++) {
    if (list_.at(i).isElement()) {
      ...
      country_->addSubElement(e);
    }
  }
  QDomNamedNodeMap attrs_ = element.attributes();
  for(int i = 0; i<attrs_.size(); i++) {
    QDomAttr a = attrs_.item(i).toAttr();
    country_->setAttribute(a);
  }
  country_->setValue(element.text());
  return country_;
}
void Country::addSubElement( QDomElement &element) {
}
void Country::setAttribute( QDomAttr &attr) {
...
QDomDocument * Country::loadXmlDocument(QFile & file) {
  if (!file.exists()) {
...
Country  * Country::fromFile(QFile & file) {
  QDomDocument *doc = Country::loadXmlDocument(file);
  if(doc==NULL) {
    return NULL;
  }
  QDomElement root = doc->documentElement();
  return Country::fromElement(root);
}
